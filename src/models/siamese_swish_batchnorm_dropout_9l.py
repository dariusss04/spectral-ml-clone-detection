# 9-layer Siamese MLP with Swish (SiLU) + BatchNorm + Dropout.

import torch
import torch.nn as nn


class SiameseNetwork_Swish_BatchNorm_Dropout_9L(nn.Module):
    """
    Siamese MLP with Swish (SiLU) + BatchNorm + Dropout.
    Inputs:  x (B, input_size)
    Outputs: embeddings (B, 64)
    """

    def __init__(self, input_size=200, dropout_prob=0.3):
        super(SiameseNetwork_Swish_BatchNorm_Dropout_9L, self).__init__()
        layers = []
