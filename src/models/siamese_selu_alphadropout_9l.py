# 9-layer Siamese MLP with SELU + AlphaDropout.

import torch
import torch.nn as nn


class SiameseNetwork_SELU_AlphaDropout_9L(nn.Module):
    """
    Siamese MLP with SELU + AlphaDropout.
    Inputs:  x (B, input_size)
    Outputs: embeddings (B, 64)
    """

    def __init__(self, input_size=200, dropout_prob=0.3):
        super(SiameseNetwork_SELU_AlphaDropout_9L, self).__init__()
        layers = []
        ...
        layers.append(nn.AlphaDropout(p=dropout_prob))
