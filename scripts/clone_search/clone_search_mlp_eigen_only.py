# Clone search using a Siamese MLP on eigenvalues only.
...
WEIGHTS_PATH = Path(os.getenv("PCD_WEIGHTS_PATH", EXPERIMENTS_DIR / "mlp_depth_sweep" / "depth_9l" / "weights.pth"))

RESULTS_DIR = Path(os.getenv("PCD_RESULTS_DIR", REPO_ROOT / "results" / "clone_search"))
RESULTS_FILE = RESULTS_DIR / "mlp_eigen_only.csv"
...
def load_vector_features(path):
    data = torch.load(path, map_location="cpu")
    vector = data["eigenvalues"].unsqueeze(0)
    return vector


def infer_input_size(state_dict):
    weight = state_dict.get("fc.1.weight")
    if weight is None:
        return 100
...
def run_clone_search(target_idx, repository_indices, model, device):
    ...
    vec_a = load_vector_features(target_path).to(device)
    ...
    with torch.no_grad():
        for repo_idx in repository_indices:
            ...
            vec_b = load_vector_features(repo_path).to(device)

            out_a, out_b = model(vec_a, vec_b)
...
def main():
    ...
    input_size = infer_input_size(state)
    model = BaseSiameseNetwork(num_layers=9, input_size=input_size)
