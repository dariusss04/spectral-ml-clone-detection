# Clone search using a Siamese MLP on eigenvalues + edge counts.
...
WEIGHTS_PATH = Path(os.getenv("PCD_WEIGHTS_PATH", EXPERIMENTS_DIR / "mlp_activation_sweep" / "leakyrelu_layernorm" / "weights.pth"))

RESULTS_DIR = Path(os.getenv("PCD_RESULTS_DIR", REPO_ROOT / "results" / "clone_search"))
RESULTS_FILE = RESULTS_DIR / "mlp_full.csv"
...
def load_vector_features(path):
    ...
    vector = torch.cat([data["eigenvalues"], data["num_edges"]], dim=0).unsqueeze(0)
    return vector
...
def main():
    ...
    model = SiameseNetwork_LeakyReLU_LayerNorm()
