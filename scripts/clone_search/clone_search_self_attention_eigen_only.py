# Clone search using self-attention with eigenvalues only.
...
from models.deepset_self_attention_eigenonly import DeepSetSelfAttentionEigenOnly
...
WEIGHTS_PATH = Path(os.getenv("PCD_WEIGHTS_PATH", EXPERIMENTS_DIR / "self_attention_eigen_only" / "weights.pth"))

RESULTS_DIR = Path(os.getenv("PCD_RESULTS_DIR", REPO_ROOT / "results" / "clone_search"))
RESULTS_FILE = RESULTS_DIR / "self_attention_eigen_only.csv"
...
def run_clone_search(target_idx, repository_indices, model, device):
    ...
    eig_a = load_set_features(target_path).to(device)
    ...
    with torch.no_grad():
        for repo_idx in repository_indices:
            ...
            eig_b = load_set_features(repo_path).to(device)

            out_a, out_b = model(eig_a, eig_b)
...
def main():
    ...
    model = DeepSetSelfAttentionEigenOnly(input_dim=1, hidden_dim=64, output_dim=64, phi_layers=20, rho_layers=12)
