# Clone search using DeepSets with eigenvalues only.
...
from models.deepset_siamese_eigenonly import SiameseNetworkDeepSetsEigenOnly
...
WEIGHTS_PATH = Path(os.getenv("PCD_WEIGHTS_PATH", EXPERIMENTS_DIR / "deepset_eigen_only" / "weights.pth"))

RESULTS_DIR = Path(os.getenv("PCD_RESULTS_DIR", REPO_ROOT / "results" / "clone_search"))
RESULTS_FILE = RESULTS_DIR / "deepset_eigen_only.csv"
...
def main():
    ...
    model = SiameseNetworkDeepSetsEigenOnly(input_dim=1, hidden_dim=64, output_dim=64, phi_layers=5, rho_layers=5)
